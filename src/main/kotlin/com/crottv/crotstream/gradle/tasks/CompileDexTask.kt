                ...
                .use { classesInput ->
                    ...
                    for (file in files) {
                        ...
                        for (annotation in classNode.visibleAnnotations.orEmpty() + classNode.invisibleAnnotations.orEmpty()) {
                            if (annotation.desc == "Lcom/crottv/crotstream/plugins/CrotstreamPlugin;") {
                                val crotstream = project.extensions.getCrotstream()

                                require(crotstream.pluginClassName == null) {
                                    "Only 1 active plugin class per project is supported"
                                }

                                crotstream.pluginClassName = classNode.name.replace('/', '.')
