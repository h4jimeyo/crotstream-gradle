...
abstract class CrotstreamExtension @Inject constructor(project: Project) {
    val userCache = project.gradle.gradleUserHomeDir.resolve("caches").resolve("crotstream")
...
class ApkInfo(extension: CrotstreamExtension, release: String) {
    val cache = extension.userCache.resolve("crotstream")

    var urlPrefix = "https://storage.googleapis.com/crotstream.appspot.com/assets/artifacts/"
    val jarFile = cache.resolve("crotstream.jar")
}
...
fun ExtensionContainer.getCrotstream(): CrotstreamExtension {
    return getByName("crotstream") as CrotstreamExtension
}

fun ExtensionContainer.findCrotstream(): CrotstreamExtension? {
    return findByName("crotstream") as CrotstreamExtension?
}